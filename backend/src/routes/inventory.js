...
// PUT /api/inventory/grn/:id — Update GRN columns, line item quantities, and prices
router.put('/grn/:id', auth, requireLevel(2), asyncRoute(async (req, res) => {
  ...
  const { vehicle_number, challan_number, invoice_number, remarks, date, items: updatedItems } = req.body;
  ...
  try {
    ...
    res.json({
      success: true,
      message: `GRN ${grn.grn_number} updated successfully`,
